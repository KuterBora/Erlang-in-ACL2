...
(define erl-add ((left erl-val-p) (right erl-val-p))
  ...
  (b* ((left (erl-val-fix left)) 
       ...
       ((unless (equal (erl-val-kind left) :integer)) (make-erl-val-excpt :err 'badarg))
       ((unless (equal (erl-val-kind right) :integer)) (make-erl-val-excpt :err 'badarg)))
...
(define erl-div ((left erl-val-p) (right erl-val-p))
  ...
  (b* ((left (erl-val-fix left)) 
       ...
       ((unless (equal (erl-val-kind left) :integer)) (make-erl-val-excpt :err 'badarg))
       ((unless (equal (erl-val-kind right) :integer)) (make-erl-val-excpt :err 'badarg))
       ((if (equal (erl-val-integer->val right) 0)) (make-erl-val-excpt :err 'badarg))
...
(define apply-erl-binop ((op erl-binop-p) (left erl-val-p) (right erl-val-p))
  ...
  (b* ((op (erl-binop-fix op))
       ...
       (right (erl-val-fix right))
       ((if (or (equal left (make-erl-val-flimit))
                (equal (erl-val-kind left) :err))) left)
       ((if (or (equal right (make-erl-val-flimit))
                (equal (erl-val-kind right) :err))) right))
      (case op
        (+ (erl-add left right))
        (- (erl-sub left right))
        (* (erl-mul left right))
        (div (erl-div left right))
        (otherwise (make-erl-val-error :err "bad op"))))
    ///
    (defrule apply-erl-binop-of-flimit
      (implies (and (erl-val-p left)
                    (erl-val-p right)
                    (erl-binop-p op)
                    (not (equal (apply-erl-binop op left right) (make-erl-val-flimit))))
