...
(defrule apply-k-of-binop-ensures
  (implies 
    (and
      ...
      (equal (kont-kind (erl-k->kont k)) :expr)
      (equal (node-kind (kont-expr->expr (erl-k->kont k))) :binop)
      (not (equal (apply-k val (cons k rest)) '(:fault)))
      (> (erl-k->fuel k) 3)
      (not (equal val (make-erl-value-fault))))
    (and (not (equal (apply-k '(:nil) (list (erl-k (+ -1 (erl-k->fuel k)) (kont-expr (node-binop->left (kont-expr->expr (erl-k->kont k))))))) '(:fault)))    
         (not (equal (apply-k '(:nil) (list (erl-k (+ -2 (erl-k->fuel k)) (kont-expr (node-binop->right(kont-expr->expr (erl-k->kont k))))))) '(:fault)))
         ))  
