...
(defrule eval-k-of-expr-tuple-empty->klst
  (implies
    (and (wf-state-p s)
         (erl-k-p k)
         (> (erl-k->fuel k) 0)
...
(defrule eval-k-of-expr-tuple-empty->s
  (implies
    (and (wf-state-p s)
         (erl-k-p k)
         (> (erl-k->fuel k) 0)
...
(defrule eval-k-of-expr-tuple->klst
  (implies
    (and (wf-state-p s)
         ...
         (equal (kont-kind (erl-k->kont k)) :expr)
         (equal (node-kind (kont-expr->expr (erl-k->kont k))) :tuple)
         (consp (node-tuple->lst (kont-expr->expr (erl-k->kont k)))))
    (equal
...
(defrule eval-k-of-tuple-merge-ok->s
  (implies
    (and (wf-state-p s)
         (erl-k-p k)
         (> (erl-k->fuel k) 0)
...
(defrule eval-k-of-tuple-merge-bad-tuple->s
  (implies
    (and (wf-state-p s)
         (erl-k-p k)
         (> (erl-k->fuel k) 0)
...
(defrule eval-k-of-tuple-merge-incomaptible->s
  (implies
    (and (wf-state-p s)
         (erl-k-p k)
         (> (erl-k->fuel k) 0)
...
(defrule eval-k-of-tuple-merge->s
  (implies
    ...
    (equal
      ...
      (cond
        ((not (wf-state-p s)) s)
        ((zp (erl-k->fuel k)) (update-erl-state->in s (make-erl-val-flimit)))
        ((not (equal (erl-val-kind (erl-state->in s)) :tuple))
         (update-erl-state->in 
            s 
            (make-erl-val-reject :err "tuple-merge expects tuple")))
