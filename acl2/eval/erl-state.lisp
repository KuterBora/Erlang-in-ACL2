...
(define erl-state-send ((s erl-state-p) (dst pid-p) (val erl-val-p))
  :returns (rs erl-state-p)
  (b* ((s (erl-state-fix s))
       (dst (pid-fix dst))
       (val (erl-val-fix val)))
  ...
  ///
    (defcong erl-state-equiv equal (erl-state-send s dst val) 1)
    (defcong pid-equiv equal (erl-state-send s dst val) 2)
    (defcong erl-val-equiv equal (erl-state-send s dst val) 3)

    (defrule erl-state-send->fields
      (and (equal (erl-state->module (erl-state-send s d v))
                  (erl-state->module s))
           (equal (erl-state->in (erl-state-send s d v))
                  (erl-state->in s))
           (equal (erl-state->bind (erl-state-send s d v))
                  (erl-state->bind s))
           (equal (erl-state->world (erl-state-send s d v))
                  (erl-state->world s))
           (equal (erl-state->self (erl-state-send s d v))
                  (erl-state->self s)))))
